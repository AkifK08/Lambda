...
function handleElicitAction(request) {
  let template = createSimpleListPickerFromOptions(
    "Welcome to iFieldSmart Technologies!. Unite Office and field teams. From design through planning, construction and operations..",
...
function handleActionResponse(input, request) {
  ...
  //   );
  // } else
   if (ACTIONS.TEST_INTERACTIVE === input) {
    let template = createSimpleListPickerFromOptions(
      "Good to hear! just a couple questions so that i can get you connected to the right person..",
